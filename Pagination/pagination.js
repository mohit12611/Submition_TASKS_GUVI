var tableData = [
  ...
  function pagination(mySet , mypage , rows){

        let  first_ele_mypage = (mypage - 1) * rows ; 
        let  last_ele_mypage = first_ele_mypage + rows;

        let currentData = mySet.slice(first_ele_mypage,last_ele_mypage);

        let totalPages = Math.ceil(mySet.length / rows);


        return {"myCurrentSet" : currentData , "totalPages" : totalPages}
  }
  ...
  function pageButtons(totalPages){
    ...
    let buttons = document.getElementById("pagination-div");

    buttons.innerHTML = "";

    let maxLeft = (equation.currentPage - Math.floor(equation.window/2));

    let maxRight = (equation.currentPage + Math.floor(equation.window/2));

    if (maxLeft < 1){
        maxLeft = 1;
        maxRight = equation.window ;
    }

    if (maxRight > totalPages){
        maxRight = totalPages;

        maxLeft = totalPages - (equation.window -1);
    ...
    for (let page = maxLeft; page <= maxRight; page++) {
    	buttons.innerHTML += `<button value=${page} class="page btn btn-sm btn-info">${page}</button>`
    }

    if (equation.currentPage !== 1) {
        buttons.innerHTML = `<button value=${1} class="page btn btn-sm btn-info">&#171; First</button>` + buttons.innerHTML
    }

    if (equation.currentPage !== totalPages) {
        buttons.innerHTML += `<button value=${totalPages} class="page btn btn-sm btn-info">Last &#187;</button>`
    }


    let  buttonlist = document.getElementsByClassName("page");
    console.log(buttonlist.length);
    console.log(equation.currentPage);

    function displayTable(){
        let tableBody = document.getElementById("table-body");
        tableBody.innerHTML ="";
        console.log(this,this.value);
        equation.currentPage = Number(this.value);
    
        createTable();
    };

    for(let i = 0 ; i < buttonlist.length ; i++){
        buttonlist[i].addEventListener('click', displayTable, false);
    }
  ...
  function createTable(){
    ...
    let result_pagination = pagination(equation.mySet, equation.currentPage, equation.rows);

    let myObj = result_pagination.myCurrentSet ;

    for(var i=1 in myObj){
        var row = document.createElement("tr");

        var td1 = document.createElement("td");
            td1.innerHTML= myObj[i].id;

        var td2 = document.createElement("td");
            td2.innerHTML= myObj[i].name;

        var td3 = document.createElement("td");
            td3.innerHTML= myObj[i].email;
    ...
    pageButtons(result_pagination.totalPages);
